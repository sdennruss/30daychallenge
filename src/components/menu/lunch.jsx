export const lunchDetails = [
  {
    days: 1,
    link: "https://www.budgetbytes.com/lentils-with-creamy-mushroom-gravy/",
    description:
      "These Lentils with Creamy Mushroom Gravy are an easy, hearty, and comforting lunch or dinner, perfect for cold winter evenings.",
  },
  {
    days: 2,
    link: "https://www.budgetbytes.com/vegan-creamy-mushroom-ramen/",
    description:
      "This incredibly simple Vegan Creamy Mushroom Ramen is a rich and flavorful 15 minute meal that only requires a handful of ingredients! ",
  },
  {
    days: 3,
    link:
      "https://www.budgetbytes.com/creamy-coconut-curry-lentils-with-spinach/",
    description:
      "These rich, creamy, and earthy Coconut Curry Lentils are an easy and delicious vegan option for dinner or weekly meal prep.",
  },
  {
    days: 4,
    link: "https://www.budgetbytes.com/spicy-coconut-vegetable-stir-fry/",
    description:
      "This rich and spicy coconut vegetable stir fry is adaptable to whatever vegetables are lingering in your fridge, making it a great sweep the kitchen recipe!",
  },
  {
    days: 5,
    link: "https://pinchofyum.com/vegan-lettuce-wraps",
    description:
      "Happily vegan – with crispy tofu bits, saucy brown rice noodles, and a creamy sesame sauce.",
  },
  {
    days: 6,
    link:
      "https://www.goodhousekeeping.com/food-recipes/a40929/bbq-chickpea-cauliflower-flatbreads-with-avocado-mash-recipe/",
    description:
      "Consider these an alternative to your morning avacado toast. Mashed avacado with roasted cauliflower, topped with chickpeas lemon juice and a pinch of salt which make for a great quick bite.",
  },
  {
    days: 7,
    link: "https://wellandfull.com/2016/08/favorite-vegan-salad/",
    description:
      "Complete with spiced chickpeas, addicting coconut bacon, and a mouthwatering avocado dressing that's sure to be a crowd-pleaser.",
  },
  {
    days: 8,
    link:
      "https://www.goodhousekeeping.com/food-recipes/a44097/summer-pesto-pasta-recipe/",
    description:
      "Loaded with all your favorite summer veggies, this pesto pasta is light and easy on warm nights.",
  },
  {
    days: 9,
    link: "https://www.feastingathome.com/farmers-market-veggie-pasta/",
    description:
      "Farmers Market Pasta Salad with grilled zucchini, corn, cherry tomatoes and black beans. Make this summer pasta salad recipe gluten - free with optional grain - free pasta. Very adaptable! Serve it warm for dinner or chilled for midweek lunches.",
  },
  {
    days: 10,
    link: "https://www.halfbakedharvest.com/the-mean-green-detox-salad/",
    description:
      "This is the perfect salad to pack for lunch, take to a party or maybe even a picnic. The longer the salad sits and marinates in the dressing, the better it becomes.",
  },
  {
    days: 11,
    link:
      "https://whitneybond.com/thai-peanut-sweet-potato-buddha-bowl-recipe/",
    description:
      "Roasted sweet potatoes & quinoa are topped with delicious Thai peanut sauce in this easy, healthy, gluten free & vegan buddha bowl recipe!",
  },
  {
    days: 12,
    link: "https://www.thissavoryvegan.com/vegan-buffalo-cauliflower-tacos/",
    description:
      "These Vegan Buffalo Cauliflower Tacos are packed full of spicy buffalo sauce, creamy ranch, crunchy romaine and hearty avocados.",
  },
  {
    days: 13,
    link: "https://www.loveandlemons.com/pumpkin-penne-australia-book-release/",
    description:
      "Creamy vegan fall pasta made with sage, balsamic, garlic, onion and creamy cashews.",
  },
  {
    days: 14,
    link: "http://wallflowerkitchen.com/creamy-mushroom-risotto-vegan-gf/",
    description:
      "An easy, healthy weeknight meal that's super creamy and full of flavour. And no, you don’t need any cheese to achieve this creamy texture and full flavour!",
  },
  {
    days: 15,
    link: "https://minimalistbaker.com/chickpea-sunflower-sandwich/",
    description:
      "30-minute Chickpea Sunflower Sandwich with smashed chickpeas and roasted sunflower seeds in a simple dressing. Sandwiched between two slices of bread and topped with my secret garlic sauce, this is healthy lunch perfection.",
  },
  {
    days: 16,
    link:
      "https://minimalistbaker.com/collard-green-spring-rolls-sunbutter-dipping-sauce/",
    description:
      "Spring rolls with a twist! Veggies + tofu wrapped in collard greens and served with a spicy-sweet sunflower butter dipping sauce. A satisfying and healthy plant-based snack or side.",
  },
  {
    days: 17,
    link: "https://minimalistbaker.com/vegan-blt-sandwich/",
    description:
      "6-Ingredient vegan sandwich made with vegan mayo and eggplant bacon! Crisp, smoky, flavorful, and so delicious.",
  },
  {
    days: 18,
    link: "https://minimalistbaker.com/spicy-buffalo-chickpea-wraps/",
    description:
      "30-minute Buffalo Chickpea Wraps requiring only 10 ingredients! Spicy chickpeas, crunchy vegetables, and a creamy hummus dressing.",
  },
  {
    days: 19,
    link: "https://www.connoisseurusveg.com/curried-tofu-salad-wraps/",
    description:
      "Crunchy veggies, chunks of hearty tofu and sweet raisins are mixed in zesty curry mayo dressing and stuffed into tortillas to make these scrumptious wraps! Perfect for lunch or an easy dinner.",
  },
  {
    days: 20,
    link:
      "https://www.kitchentreaty.com/simple-lemon-dill-quinoa-chickpea-salad/",
    description:
      "This salad comes together quickly and easily, with only a bit of chopping, a little stirring, and a handful of ingredients.",
  },
  {
    days: 21,
    link:
      "https://www.connoisseurusveg.com/pineapple-fried-rice-with-basil-peanuts/",
    description:
      "Juicy pineapple chunks are stir-fried with crispy rice, edamame and fresh basil in this peanut-topped pineapple fried rice! Better than takeout and easy enough for lunch or a weeknight!",
  },
  {
    days: 22,
    link: "https://www.connoisseurusveg.com/vegan-muffaletta/",
    description:
      "This vegan muffaletta sandwich is packed with spicy marinated eggplant, roasted red pepper slices, and zippy olive salad! A delicious make-ahead vegan sandwich that gets better with time. This one is perfect for picnics!",
  },
  {
    days: 23,
    link: "https://www.connoisseurusveg.com/vegan-cobb-salad/",
    description:
      "Juicy tomatoes, creamy avocado slices, hearty chickpeas, smoky coconut bacon, and savory tofu chunks are arranged atop crispy greens to make this flavor-packed and super satisfying vegan cobb salad.",
  },
  {
    days: 24,
    link: "https://www.connoisseurusveg.com/tempeh-chili/",
    description:
      "Hearty pinto beans, fire roasted tomatoes, and lots of smoky and spicy seasonings go into this satisfying and delicious vegan tempeh chili.",
  },
  {
    days: 25,
    link: "https://www.connoisseurusveg.com/pineapple-stir-fry/",
    description:
      "This tofu and pineapple stir-fry is packed with sweet and spicy flavor and comes together lightning fast! Easy, delicious, and perfect for lunch or a weeknight dinner.",
  },
  {
    days: 26,
    link:
      "https://www.ambitiouskitchen.com/mango-avocado-black-rice-salad-with-cilantro-lime-vinaigrette-healthy-gluten-free-vegan/",
    description:
      "Bright and fresh, this incredible black rice salad isn’t only flavorful, it’s packed with nutrients too. It’s sweet, creamy, and crunchy, and topped with an addicting cilantro-lime vinaigrette. You’ll love this for a healthy lunch.",
  },
  {
    days: 27,
    link:
      "https://www.ambitiouskitchen.com/smashed-chickpea-avocado-salad-sandwich-with-cranberries-lemon/",
    description:
      "Eliminate mayo in this recipe as it's unnecessary all thanks to the creamy, ripe avocado. Cranberries and lemon give it a nice sweet tang! You’ll love how easy this is to throw together for lunch.",
  },
  {
    days: 28,
    link:
      "https://www.ambitiouskitchen.com/vegan-macadamia-coconut-tofu-bowls/",
    description:
      "Vegan tofu bowls with sweet potato coconut milk sauce, spinach, sweet buttery macadamia nuts and brown rice. Made with organic, extra firm tofu. The perfect veggie packed lunch for meal prep!",
  },
  {
    days: 29,
    link:
      "https://www.ambitiouskitchen.com/strawberry-mango-chopped-spinach-quinoa-salad-with-sesame-lime-vinaigrette/",
    description:
      "Nutritious and flavorful spinach quinoa salad bursting with mango, strawberries, avocado and a scrumptious sesame-lime vinaigrette. Tangy and sweet, it makes an amazing side or meal that’s perfect for parties.",
  },
  {
    days: 30,
    link: "https://veganheaven.org/recipe/vegan-basil-ricotta-pinwheels/",
    description:
      "These vegan basil ricotta pinwheels are perfect for a quick and healthy lunch. They're also a real crowd pleaser at parties and family get-togethers.",
  },
];

export default lunchDetails;
